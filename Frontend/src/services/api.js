...
const apiService = {
  ...
  async getRecentCharacters(limit = 10) {
    try {
      ...
      response.data.forEach((char, index) => {
        logger.debug(`[API] Personagem recente ${index + 1} (${char.name}):`, {
          id: char.id,
          name: char.name,
          ...
          previous_experience_date: char.previous_experience_date,
          exp_gained: char.exp_gained,
          exp_gained_period: char.exp_gained_period,
          level: char.level,
  ...
  async getCharactersByIds(ids) {
    try {
      ...
      // Log each character for debugging
      response.data.forEach((char, index) => {
        logger.debug(`[API] Personagem ${index + 1} (${char.name}):`, {
          id: char.id,
          name: char.name,
          level: char.level,
          vocation: char.vocation,
