...
# =============================================================================
# TIBIA TRACKER - SCRIPT DE BACKUP DO BANCO DE DADOS
# =============================================================================
# Script para fazer backup completo do banco PostgreSQL
# Executa backup antes de qualquer modificação no banco
...
log "Banco: ${DB_NAME:-tibia_tracker}"
log "Host: ${DB_HOST:-localhost}"
log "Porta: ${DB_PORT:-5432}"
log "Usuário: ${DB_USER:-tibia_user}"
...
docker exec tibia-tracker-postgres pg_dump \
    -U "${DB_USER:-tibia_user}" \
    -d "${DB_NAME:-tibia_tracker}" \
    -h localhost \
    -p 5432 \
    --verbose \
    --clean \
...
Tamanho: $BACKUP_SIZE
Banco: ${DB_NAME:-tibia_tracker}
Host: ${DB_HOST:-localhost}
Porta: ${DB_PORT:-5432}
Usuário: ${DB_USER:-tibia_user}

# Comando para restaurar:
# gunzip -c $COMPRESSED_BACKUP | docker exec -i tibia-tracker-postgres psql -U ${DB_USER:-tibia_user} -d ${DB_NAME:-tibia_tracker}

# Comando para verificar:
# docker exec tibia-tracker-postgres psql -U ${DB_USER:-tibia_user} -d ${DB_NAME:-tibia_tracker} -c "SELECT COUNT(*) FROM characters;"
EOF
...
log "  gunzip -c $COMPRESSED_BACKUP | docker exec -i tibia-tracker-postgres psql -U ${DB_USER:-tibia_user} -d ${DB_NAME:-tibia_tracker}"
echo "" 