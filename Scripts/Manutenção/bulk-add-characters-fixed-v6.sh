...
# =============================================================================
# SCRIPT TEMPORÁRIO - ADIÇÃO EM MASSA DE PERSONAGENS (VERSÃO CORRIGIDA V6)
# =============================================================================
...
add_character() {
    ...
    # Remover espaços extras, quebras de linha e caracteres especiais do nome
    ...
    # Limpar quebras de linha e fazer encode URI
    local clean_server=$(echo "$server" | tr -d '\r\n' | jq -sRr @uri)
    local clean_world=$(echo "$world" | tr -d '\r\n' | jq -sRr @uri)
    local clean_name=$(echo "$name" | tr -d '\r\n' | jq -sRr @uri)
    
    local url="$API_URL/api/v1/characters/scrape-and-create?server=$clean_server&world=$clean_world&character_name=$clean_name"
...
# Função principal
main() {
    log "=== INICIANDO ADIÇÃO EM MASSA DE PERSONAGENS (VERSÃO CORRIGIDA V6) ==="
