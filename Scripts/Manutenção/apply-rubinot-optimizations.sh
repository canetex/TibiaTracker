...
# Script para Aplicar Otimizações do Rubinot
# ==========================================
...
docker exec tibia-tracker-postgres pg_dump -U tibia_tracker_user tibia_tracker_db > "$BACKUP_FILE"
log "✅ Backup criado: $BACKUP_FILE"

# 2. Aplicar script de otimização
log "🔧 Aplicando otimizações do banco de dados..."
docker exec -i tibia-tracker-postgres psql -U tibia_tracker_user -d tibia_tracker_db < Backend/sql/optimize_for_rubinot.sql
...
docker exec tibia-tracker-postgres psql -U tibia_tracker_user -d tibia_tracker_db -c "
SELECT 
    schemaname,
    tablename,
    indexname
...
echo "   - Estatísticas do banco: docker exec tibia-tracker-postgres psql -U tibia_tracker_user -d tibia_tracker_db -c \"SELECT * FROM pg_stat_user_tables;\""
