...
# =============================================================================
# SCRIPT TEMPORÁRIO - ADIÇÃO EM MASSA DE PERSONAGENS (VERSÃO CORRIGIDA V2)
# =============================================================================
...
test_api_connectivity() {
    ...
    if ! curl -s -f "$API_URL/api/v1/characters/stats/global" > /dev/null 2>&1; then
        error "❌ Endpoint de personagens não está respondendo"
        return 1
    fi
    
    # Testar endpoint específico de scrape-and-create com prefixo correto
    if ! curl -s -f "$API_URL/api/v1/characters/scrape-and-create?server=taleon&world=san&character_name=test" > /dev/null 2>&1; then
...
# Função principal
main() {
    log "=== INICIANDO ADIÇÃO EM MASSA DE PERSONAGENS (VERSÃO CORRIGIDA V2) ==="
